...
export const scrapeArchivedCourses = async (env: Env, semester: string) => {
    const landingPageRes = await fetch("https://www.ccxp.nthu.edu.tw/ccxp/INQUIRE/JH/6/6.2/6.2.9/JH629001.php", { cf: { cacheTtl: 0 } })
    ...
    const acixStoreMatch = landingPageRes.match(/auth_img\.php\?ACIXSTORE=([a-zA-Z0-9]+)/);
    if (!acixStoreMatch) {
    ...
    optionElements.forEach((option, index) => {
        ...
        if (!value || value === '' || text?.startsWith('開課代號') || text?.includes('請選擇開課代號')) {
            return;
        }
    ...
    departments = departments.filter(department => !skippedDepartments.includes(department.code.trim()));
    ...
    for (const chunk of chunked) {
        const { error } = await supabaseWithEnv(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY).from("courses").upsert(chunk);
...
export const scrapeSyllabus = async (env: Env, semester: string, cachedCourses?: Course[]) => {
    ...
    const processCourse = async (course: any) => {
        ...
        const { brief: _brief, keywords, content: _content } = await parseContent(env, html, raw_id);
        ...
        const { error } = await supabaseWithEnv(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY).from("course_syllabus").upsert({
            raw_id,
            brief,
        ...
        if (cachedCourses) {
            ...
            algoliaWithEnv(env.ALGOLIA_APP_ID, env.ALGOLIA_API_KEY).saveObject(algoliaCourse);
        }
    ...
    console.log(`Scraped syllabus for ${courses.length} courses in semester ${semester}`);
};

export const syncCoursesToAlgolia = async (env: Env, semester: string) => {
    const query = await supabaseWithEnv(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY)
        .from("courses")
    ...
    for (const chunk of chunked) {
        ...
        algoliaWithEnv(env.ALGOLIA_APP_ID, env.ALGOLIA_API_KEY).saveObjects(algoliaChunk);
    }
