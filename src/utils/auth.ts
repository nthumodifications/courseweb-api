...
export const auth = (requiredScopes?: string[]): MiddlewareHandler => {
    return async (c: Context, next: Next) => {
        ...
        try {
            const { NTHUMODS_AUTH_INTROSPECTION_URL, NTHUMODS_AUTH_CLIENT_ID, NTHUMODS_AUTH_CLIENT_SECRET } = env<{
            ...
            const hasRequiredScope = requiredScopes.some(requiredScope => {
                ...
                return userScopes.includes(requiredScope) || userScopes.includes(baseScope);
            });
